...
union
{
  ...
  BUFFER_JPEG jpg;              /* jpeg image buffer */
};
...
size_t nbytes;
  ...
  {
  	nbytes = (INPUT_BUF_SIZE > ( src->jpg.buffer_size - src->jpg.pos )? 
  ...
  src->pub.bytes_in_buffer = nbytes;
#else
  memcpy(src->buffer,src->jpg.img_buffer,src->jpg.buffer_size );
  src->pub.bytes_in_buffer = src->jpg.buffer_size;
#endif
  src->pub.next_input_byte = src->buffer;
  src->start_of_file = FALSE;
